...
public class ConcurrentUpdateExceptionHandler {
    ...
    public ResponseEntity<Map<String, Object>> handleConcurrentUpdate(OptimisticLockingFailureException ex) {
        ...
        return conflict("Kayıt başka bir işlem tarafından değiştirildi. Sayfayı yenileyip tekrar deneyin.");
    }

    @ExceptionHandler(DataIntegrityViolationException.class)
    public ResponseEntity<Map<String, Object>> handleDataIntegrityViolation(DataIntegrityViolationException ex) {
        log.warn("Veri bütünlüğü çakışması: {}", ex.getMostSpecificCause().getMessage());
        return conflict("Bu işlem mevcut bir kayıtla çakışıyor. Sayfayı yenileyip tekrar deneyin.");
    }
